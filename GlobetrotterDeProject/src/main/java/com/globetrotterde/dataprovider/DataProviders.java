...
package com.globetrotterde.dataprovider;
...
import com.globetrotterde.utility.NewExcelLibrary;
...
public class DataProviders {
	...
	//Class --> AddToCartPageTest, EndToEndTest,  Test Case--> addToCartTest, endToEndTest	
		...
		//To update Billing Address - Phonenumber having spaces and +
		@DataProvider(name = "editPhone")
		public Object[][] getPhoneNumber() {
			// Totals rows count
			int rows = obj.getRowCount("BillingAddress");
			// Total Columns
			int column = obj.getColumnCount("BillingAddress");
			int actRows = rows - 1;
			...
			for (int i = 0; i < actRows; i++) {
				for (int j = 0; j < column; j++) {
					data[i][j] = obj.getCellData("BillingAddress", j, i + 2);
				}
			}
			return data;
		}
		...
		//Credit Card Details
		@DataProvider(name = "payCreditCardDetailsData")
		public Object[][] creditPayDetails() {

			// Totals rows count
			int rows = obj.getRowCount("CreditCardDetails");
			// Total Columns
			int column = obj.getColumnCount("CreditCardDetails");
			int actRows = rows - 1;
			//Created an object of array to store data
			Object[][] data = new Object[actRows][1];
			...
			for (int i = 0; i < actRows; i++) {
				Map<String, String> hashMap = new HashMap<>();
				for (int j = 0; j < column; j++) {
					hashMap.put(obj.getCellData("CreditCardDetails", j, 1),
							obj.getCellData("CreditCardDetails", j, i + 2));
				}
				data[i][0]=hashMap;
			}
			return data;
